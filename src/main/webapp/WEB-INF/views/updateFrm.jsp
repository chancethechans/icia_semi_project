...
<div class="wrap">
	...
	<div class="content">
		<form action="./boardUpdate" class="write-form"
			...
			<!-- 파일을 업로드 하기 위해서 form에 enctype="multipart/form-data" 속성을 추가 -->
			<div class="user-info">
			...
			<input type="hidden" name="bnum" value="${board.b_num}"> <!-- 얘가 있어야 where 조건 만족 -->
			<input type="text" class="write-input"
				name="btitle" autofocus placeholder="제목" required value="${board.b_title}">
			...
			<!-- ***첨부된 파일 출력 영역 -->
				<div id="bfile" style="margin-bottom: 10px;">
					...
					<c:if test="${empty fList}">
					 	<label style="width: 100%">첨부 파일 없음</lable>
					</c:if>
			...
			<!-- 파일 입력 처리 영역 --> <!-- label for 는 input id와 연결 -->
...
<script type="text/javascript">
//업로드할 파일을 선택하면, 'upload-name' 요소에 파일 이름을 출력하고 'fileCheck' 요소의 value를 1로 변경
	...
	if (con == true) {
		...
		$.ajax({ 
			...
			success: function(result){
				...
				if(rflist.length == 0) {
					flist += '<label style="width: 100%">첨부 파일 없음</lable>';
				}
				else {
